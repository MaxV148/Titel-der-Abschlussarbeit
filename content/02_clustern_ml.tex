...
\subsection{Bag-of-Words: Vorteile und Grenzen hochdimensionaler Vektorräume}

Ein grundlegender Ansatz ist das Bag-of-Words-Modell.\footnote{Der Artikel von IBM klärt über das Bag-of-Words-Modell auf: \url{https://www.ibm.com/de-de/think/topics/bag-of-words} (Zugriff: \today)} Es basiert auf der Konstruktion eines Vektorraums, in dem jedes eindeutige Wort, das im Textkorpus vorkommt, eine eigene Dimension bildet. Umfasst das Vokabular des gesamten Korpus beispielsweise $n$ unterschiedliche Wörter, so weist der resultierende Vektorraum ebenfalls $n$ Dimensionen auf. Die Dimensionalität hängt somit von der Größe des Vokabulars ab, also von der Anzahl unterschiedlicher Wörter über alle Dokumente hinweg, und nicht direkt von der Anzahl oder dem Umfang der Dokumente: Ein Korpus aus vielen, aber sprachlich ähnlichen Dokumenten -- etwa sich wiederholenden Log-Nachrichten -- kann ein kleineres Vokabular und damit eine geringere Dimensionalität aufweisen als ein Korpus aus wenigen, aber sprachlich vielfältigen Dokumenten. Innerhalb dieses mehrdimensionalen Raums wird jedes Textdokument als einzelner Datenpunkt verortet. Die Position des Punktes entlang einer bestimmten Achse (Dimension) wird dabei durch die Häufigkeit des entsprechenden Wortes im Dokument bestimmt: Begriffe, die besonders oft auftreten, haben einen stärkeren Einfluss auf die Positionierung des Dokuments im Vektorraum, ohne dass sich dadurch die Anzahl der Dimensionen selbst ändert. Bei umfangreichen, vokabularreichen Textsammlungen führt dies in Summe zu einer sehr hohen Dimensionalität, die sowohl Chancen als auch Herausforderungen für die weiterführende Analyse mit sich bringt.


Risiken sind dabei:
\begin{itemize}
    \item Überanpassung (Overfitting): Ein großes, vielfältiges Vokabular führt zu sehr vielen Dimensionen, was Performance einbüßen und hohen Speicherverbrauch zur Folge hat.
...
Im Kontext klassischer Algorithmen hat sich zur Realisierung einer ressourcenschonenden und zugleich präzisen Gewichtung das \ac{TFIDF}-Maß etabliert. Diese statistische Methode quantifiziert die Relevanz eines einzelnen Wortes innerhalb eines Dokuments in Relation zur gesamten Dokumentenkollektion. Ein wesentlicher Vorteil dieses Verfahrens liegt in der Überwindung der Schwächen reiner Häufigkeitszählungen: Während allgemein häufige Begriffe in ihrer Gewichtung herabgestuft werden, erfahren seltene, aber inhaltlich signifikante Terme – wie beispielsweise spezifische Fehlercodes – eine stärkere Gewichtung. Basis dieser Berechnung ist die sogenannte „Term Frequency“, welche die Häufigkeit eines Wortes in einem betrachteten Dokument beschreibt. Formal lässt sich dies wie folgt darstellen \cite[Kap.~11]{jm3}:
...
Die Logarithmusfunktion vermeidet eine übermäßige Gewichtung von Wörtern mit hoher Vorkommensfrequenz. Der zweite Teil des Tf-IDF heißt \ac{IDF} \cite[Kap.~11]{jm3}:
\begin{equation}
...
Während klassische Ansätze oft statische Embeddings für einzelne Wörter generieren, ist es für die Verarbeitung ganzer Textpassagen notwendig, den Kontext dynamisch zu erfassen \cite[Kap.~5.5]{jm3}. Zur praktischen Vektorisierung der Eingabedaten wird daher auf das Framework \texttt{sentence-transformers} (\acs{SBERT})\footnote{Webseite der Python Bibliotek: \url{https://www.sbert.net/} (Zugegriffen: \today)} zurückgegriffen (vgl. sbert.net). Dieses Framework ermöglicht die Generierung von Satz-Embeddings, die speziell für semantische Ähnlichkeitsvergleiche optimiert sind.
...
Der K-Means-Clusteralgorithmus umfasst drei Schritte. Die Nutzung erfordert zunächst die Wahl eines Wertes für k. Dabei gilt $K \leq N$, wobei $N$ die Anzahl der Datenpunkte (Log-Nachrichten) im Datensatz bezeichnet, da jedem Cluster mindestens ein Datenpunkt zugeordnet sein muss.
...
\begin{enumerate}
    ...
    \item Im zweiten Schritt berechnet er für jeden Vektor $x^{(n)}$ die Distanz zu den $K$ Masseschwerpunkten. Die Distanzfunktion hängt von der Problemstellung ab. Oft nutzt man die euklidische Distanz, im Folgenden einheitlich mit dem Normsymbol $\|\cdot\|$ bezeichnet \cite{Wilmott2020-rx}:
    \begin{equation}
      \text{Distanz}(n,k) = \left\| x^{(n)} - c^{(k)} \right\| = \sqrt{ \sum_{m=1}^{M} \left( x_m^{(n)} - c_m^{(k)} \right)^2 } \quad \text{für } k=1 \text{ bis } K
      \label{eq:distanz}
    \end{equation}
    Dabei bezeichnet $M$ die Dimensionalität des verwendeten Vektorraums (vgl. Abschnitt~\ref{sec:auswahl}) und $\|\cdot\|$ die euklidische Norm, also die Wurzel der Summe der quadrierten Komponentendifferenzen. Jeder Datenpunkt erhält den nächstgelegenen Masseschwerpunkt als Zuordnung.
...
Der manuelle Aufwand bei einem System, das periodisch neue Logs einliest, ist jedoch zu hoch. Selbst eine automatisierte, datengetriebene Bestimmung von K mittels des Silhouette-Scores erforderte bei jedem Durchlauf eine Neuberechnung. Zudem ist die Anzahl der Loggruppen im Voraus unbekannt. Deshalb ist ein Cluster-Algorithmus erforderlich, der ohne feste Eingabeparameter wie K auskommt.

\section{DBSCAN: Flexibles, dichtebasiertes Clustering ohne vordefinierte Cluster-Anzahl}
\label{sec:dbscan}

Der DBSCAN-Algorithmus gehört zu den dichtebasierten Clusterverfahren. Anders als k-Means benötigt er keinen Parameter k, was eine flexiblere Log-Gruppierung ermöglicht. DBSCAN definiert einen Cluster als dichte Region aus Datenpunkten, getrennt durch Bereiche geringerer Dichte. Der Algorithmus unterteilt Datenpunkte in drei Kategorien: Kernpunkte, Grenzpunkte und Rauschpunkte. Sei $D = \{x^{(1)}, \dots, x^{(N)}\}$ der Datensatz aus $N$ Vektoren mit $x \in \mathbb{R}^d$, wobei $\mathbb{R}$ die Menge der reellen Zahlen und $d$ die Dimensionalität des Vektorraums bezeichnet (entspricht $M$ aus Gleichung~\eqref{eq:distanz}). Eine dichte Region definiert sich als Radius $\varepsilon$ um einen Punkt $x$ \cite{Mohammed_Wagner_2020}:
\begin{equation}
    N_{\epsilon}(\mathbf{x}) = B_{d}(\mathbf{x}, \epsilon) = \{ \mathbf{y} \in D \mid \| \mathbf{x} - \mathbf{y} \| \leq \epsilon \}
    \label{eq:eps_umgebung}
\end{equation}
Dabei bezeichnet $N_{\epsilon}(\mathbf{x})$ die $\epsilon$-Umgebung von $x$, also die Menge aller Punkte $y$ aus dem Datensatz $D$, für die gilt (Bedingungsstrich $\mid$), dass ihre Distanz $\| \mathbf{x} - \mathbf{y} \|$ zu $x$ höchstens $\epsilon$ beträgt; $\|\cdot\|$ ist dabei dieselbe euklidische Norm wie in Gleichung~\eqref{eq:distanz}, ersetzbar durch andere Distanzfunktionen. $B_{d}(\mathbf{x}, \epsilon)$ beschreibt dieselbe Menge geometrisch als $d$-dimensionale Kugel mit Radius $\epsilon$ um $x$. Der Parameter $\epsilon$ gibt also den Radius dieser Kugel an. Zusätzlich definiert der Parameter $\textit{minPts}$ einen Schwellwert für die Mindestanzahl an Nachbarn innerhalb des Radius $\epsilon$: Ein Datenpunkt $x \in D$ heißt Kernpunkt, wenn $N_\epsilon(x)$ mindestens $\textit{minPts}$ Punkte enthält. Hat ein Punkt weniger Nachbarn, gilt er als Grenzpunkt. Liegt ein Datenpunkt zwar in Reichweite, aber nicht im $\epsilon$-Radius eines Kernpunktes, ist er ein Rauschpunkt. DBSCAN arbeitet in folgenden Schritten \cite{geeksforgeeksDBSCANClustering}:
\begin{enumerate}
    \item Kernpunkte identifizieren: Für jeden Datenpunkt $x \in D$ zählt der Algorithmus die Punkte innerhalb des Radius $\epsilon$. Übersteigt die Anzahl $\textit{minPts}$, gilt der Punkt als Kernpunkt.
...
Drain ist in der Lage, Lognachrichten kontinuierlich zu verarbeiten und sie dabei einer Loggruppe zuzuordnen. Eine Loggruppe fasst alle Lognachrichten zusammen, die demselben Ereignistyp entsprechen, und besitzt ein zugehöriges Logtemplate -- ein generalisiertes Muster der Nachricht, in dem variable Bestandteile durch Wildcards (\texttt{<*>}) ersetzt sind. In der praktischen Implementierung \texttt{drain3} wird eine Loggruppe durch die Klasse \texttt{LogCluster} abgebildet (vgl. Abschnitt~\ref{sec:drain3_persistenz}). Ein Online-Logparser wie \texttt{drain3} kann Logs eines Systems „on-demand“ verarbeiten, er muss nicht vorher auf Daten angelernt werden. Dies geschieht durch einen internen Parse-Baum mit fester Tiefe (Fixed Depth Tree), welcher den Kern des Algorithmus bildet. Statt Lognachrichten erst über einen Zeitraum zu sammeln und nachträglich auszuwerten, leitet Drain jede neu eintreffende Nachricht direkt durch diese Baumstruktur, um sie effizient dem passenden Cluster zuzuordnen. Der Prozess lässt sich vereinfacht in vier Schritte unterteilen \cite[Abschn.~III]{drain}:
...
gespeicherte Logtemplate einer der potenziellen Loggruppen am Blattknoten. Der Ausdruck $seq(i)$ bezeichnet
...
$simSeq \geq st$, wird die Nachricht der Loggruppe mit dem höchsten
...
Build-Systemen wie Jenkins, wo der Projektname den Anfang einer Logzeile bildet.\cite[C. Limitations of Existing Work]{brain}
...
\section{MinHash und Locality Sensitive Hashing}
\label{sec:minhash_lsh}

MinHash und Locality Sensitive Hashing (\acs{LSH}) bilden eine Kombination probabilistischer Algorithmen zur effizienten Auffindung ähnlicher Datenpunkte in großen Mengen. Das Verfahren opfert ein geringes Maß an Präzision zugunsten einer erheblichen Reduktion des Rechenaufwands. Klassische Einsatzgebiete sind die Duplikaterkennung in Dokumentensammlungen sowie kollaborative Filtersysteme im großflächigen E-Commerce \cite{Leskovec2020_Kap3_1}. Im Vergleich zu den vorgestellten partitionierenden und dichtebasierten Verfahren stellt MinHash\,+\,LSH einen rein hashbasierten Ansatz dar, dessen Eignung für das kontinuierliche Log-Clustering im Folgenden theoretisch eingeordnet und in Abschnitt~\ref{sec:eval:param:minhash} sowie Abschnitt~\ref{sec:eval:minhash} empirisch überprüft wird.

\paragraph{Shingling.}
Im ersten Schritt wird jede Log-Nachricht durch \textit{Shingling} in eine mathematische Menge überführt. Ein $k$-Shingle bezeichnet einen beliebigen Teilstring der Länge $k$, der in der Nachricht vorkommt. Eine Lognachricht $d$ wird damit auf die Menge $S_d \subseteq \Sigma^{k}$ aller in ihr enthaltenen $k$-Shingles abgebildet, wobei $\Sigma$ das zugrundeliegende Token-Alphabet bezeichnet. Die Wahl von $k \in \mathbb{N}_{>0}$ beeinflusst die Trennschärfe des Verfahrens maßgeblich: Zu kleine Werte führen zu fälschlich hohen Ähnlichkeitswerten, da bereits einzelne gemeinsame Tokens eine große Überschneidung erzeugen \cite{Leskovec2020_Kap3_3_2}.


Als Beispiel dient diese Lognachricht: \textbf{sshd[14144]: Accepted publickey for ubuntu}. Für $k = 3$ ergibt sich folgende Menge an 40 $k$-Shingles
\shorthandoff{"}
\begin{equation}
\begin{split}
\{\ &\texttt{"ssh"}, \texttt{"shd"}, \texttt{"hd["}, \texttt{"d[1"}, \texttt{"[14"}, \texttt{"141"}, \texttt{"414"}, \texttt{"144"}, \\
&\texttt{"44]"}, \texttt{"4]:"}, \texttt{"]: "}, \texttt{": A"}, \texttt{" Ac"}, \texttt{"Acc"}, \texttt{"cce"}, \texttt{"cep"}, \\
&\texttt{"ept"}, \texttt{"pte"}, \texttt{"ted"}, \texttt{"ed "}, \texttt{"d p"}, \texttt{" pu"}, \texttt{"pub"}, \texttt{"ubl"}, \\
&\texttt{"bli"}, \texttt{"lic"}, \texttt{"ick"}, \texttt{"cke"}, \texttt{"key"}, \texttt{"ey "}, \texttt{"y f"}, \texttt{" fo"}, \\
&\texttt{"for"}, \texttt{"or "}, \texttt{"r u"}, \texttt{" ub"}, \texttt{"ubu"}, \texttt{"bun"}, \texttt{"unt"}, \texttt{"ntu"}\ \}
\end{split}
\label{eq:shingling_example}
\end{equation}
\shorthandon{"}


\paragraph{Jaccard-Ähnlichkeit.}
Als Ähnlichkeitsmetrik dient die Jaccard-Ähnlichkeit, die das Verhältnis von Schnitt- zu Vereinigungsmenge zweier Shingle-Mengen $S$ und $T$ beschreibt:
\begin{equation}
    \mathrm{SIM}(S, T) = \frac{|S \cap T|}{|S \cup T|}
    \label{eq:jaccard}
\end{equation}
Dabei sind $S, T$ endliche Mengen von $k$-Shingles und $|\cdot|$ bezeichnet die Kardinalität. Der Wertebereich beträgt $\mathrm{SIM}(S, T) \in [0, 1]$: Der Wert $0$ entspricht disjunkten Mengen ($S \cap T = \emptyset$), der Wert $1$ identischen Mengen ($S = T$).

\paragraph{MinHashing.}
Der direkte mengenbasierte Vergleich erzeugt bei $n$ Lognachrichten einen Aufwand von $\mathcal{O}(n^2)$. \textit{MinHashing} reduziert die Mengen daher auf kompakte Signaturen fester Länge. Der Algorithmus permutiert die Zeilen der booleschen Shingle-Matrix zufällig und speichert je Spalte den Index der ersten Zeile mit dem Wert~1. Die Wahrscheinlichkeit, dass zwei Spalten nach einer Permutation denselben MinHash-Wert aufweisen, entspricht exakt ihrer Jaccard-Ähnlichkeit.
Hierbei bezeichnet $\pi$ eine zufällige Permutation der Zeilen und $h_{\pi}(\cdot)$ die zugehörige MinHash-Funktion. Durch $p$-fache Wiederholung mit unabhängigen Permutationen entsteht eine Signatur der Länge $p \in \mathbb{N}_{>0}$. Die mittlere Anzahl übereinstimmender Positionen zweier Signaturen approximiert die Jaccard-Ähnlichkeit erwartungstreu \cite{Leskovec2020_Kap3_3}. In der späteren Implementierung entspricht $p$ dem Parameter \texttt{MINHASH\_NUM\_PERM} (vgl.\ Abschnitt~\ref{sec:eval:minhash}).

\paragraph{Locality Sensitive Hashing.}
Auch mit Signaturen verbleiben bei vollständigem Vergleich $\mathcal{O}(n^2)$ paarweise Operationen. Das Banding-Verfahren des LSH unterteilt die Signaturmatrix horizontal in $b$ Bänder zu je $r$ Zeilen, sodass $p = b \cdot r$ gilt. Jeder Band-Teilvektor wird per Hashfunktion auf einen Bucket abgebildet. Zwei Signaturen gelten als Kandidatenpaar, sobald sie in mindestens einem Band in denselben Bucket fallen. Die Kandidatenwahrscheinlichkeit bei Jaccard-Ähnlichkeit $s$ beträgt:
\begin{equation}
    P(\text{Kandidat}) = 1 - (1 - s^{r})^{b}
    \label{eq:lsh_prob}
\end{equation}
Die Parameter sind $s \in [0, 1]$ (Jaccard-Ähnlichkeit des Paares), $r \in \mathbb{N}_{>0}$ (Zeilen pro Band) und $b \in \mathbb{N}_{>0}$ (Anzahl der Bänder); die Funktion liefert einen Wert in $[0, 1]$. Durch Variation von $b$ und $r$ lässt sich die Schwellwertkurve so steuern, dass der Übergang von \textit{kein Kandidat} zu \textit{Kandidat} einem annähernden Stufensprung gleicht \cite{Leskovec2020_Kap3_4_2}.

\paragraph{Limitierungen für kontinuierliches Clustering.}
Aus der Konstruktion ergeben sich zwei strukturelle Einschränkungen für den vorliegenden Anwendungsfall. Erstens ist MinHash\,+\,LSH ein Batch-Verfahren: Die Signaturmatrix und der LSH-Index werden über den vollständigen Datensatz aufgebaut; ein persistierbares Gedächtnis -- wie es Drain3 (vgl.\ Abschnitt~\ref{sec:drain3}) durch den serialisierten Parse-Baum bereitstellt -- existiert nicht. Zweitens misst die Jaccard-Ähnlichkeit über Wort-N-Gramme die strukturelle Zeichenüberschneidung, nicht die semantische Verwandtschaft von Log-Templates. Variable Bestandteile wie IP-Adressen, Zeitstempel oder numerische Identifier erzeugen bei Logs desselben Templates unterschiedliche Shingle-Mengen und senken die gemessene Ähnlichkeit ab. Die empirische Auswirkung dieser beiden Einschränkungen auf den verwendeten Loghub-Datensatz wird in Abschnitt~\ref{sec:eval:param:minhash} (Parametersuche) und Abschnitt~\ref{sec:eval:minhash} (Ergebnisse) dokumentiert.