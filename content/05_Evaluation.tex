...
erfolgt nicht heuristisch, sondern datengetrieben über zwei Verfahren: die Elbow-Methode mit automatischer Knickerkennung sowie den Silhouette-Score.
...
Der Knickpunkt (punkt mit maximalem Krümmung) in der Elbow-Kurve wird automatisch über die
...
Endpunkte und der normierten Kurve. Dabei wird auf der Y-Ache der WCSS, auch inertia genannt, \cite{Rallabandi} Wert abgetragen und auf der X-Achse die Anzahl der Cluster.  Die Parameter lauten
\texttt{curve=\textquotesingle convex\textquotesingle},
\texttt{direction=\textquotesingle decreasing\textquotesingle}, entsprechend
dem typischen Verlauf einer fallenden Elbow-Kurve. Dise Paramater wurde durch die von der Bibliothek bereitgestellten hilfsfunktion \texttt{find\_shape} automatisch ermittelt.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{images/k_means_ellenbow.png}
    \caption{Elbow-Kurve über $k \in [2, 39]$. Auf der Y-Achse ist die Within-Cluster Sum of Squares (\acs{WCSS}) aufgetragen. Die grüne gestrichelte Linie markiert den automatisch erkannten Knee bei $k = 12$.}
...
den Knick bei $k = 12$. In Abbildung~\ref{fig:kmeans-silhouette} erreicht der
Silhouette-Score sein globales Maximum dagegen bereits bei $k = 10$; nach
einem Abfall bis etwa $k = 22$ steigt die Kurve zu größeren $k$ hin wieder
leicht an, ohne das Maximum bei $k = 10$ zu übertreffen.

Die beiden Heuristiken konvergieren in diesem Fall nicht auf einen
gemeinsamen Wert. Da der Silhouette-Score die Kompaktheit und Separation der
resultierenden Partitionen direkt bewertet, während die Elbow-Methode
lediglich einen Kompromiss zwischen Modellkomplexität und Varianzreduktion
markiert, wird dem Silhouette-Optimum der Vorzug gegeben. Es wird daher
$k = 10$ für die in Listing~\ref{lst:k-means-cluster} dokumentierte
Implementierung sowie für die nachfolgende Evaluation verwendet.
...
verwendet ($n = 4\,000$, Seed $42$); die Anzahl der enthaltenen
Ground-Truth-Klassen beträgt entsprechend ebenfalls $42$. Die im
...
der Ground-Truth-Klassenzahl der Stichprobe ($42$). Die zugehörige
Implementierung findet sich im Notebook \texttt{ML.ipynb} (Zelle~6).

\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\linewidth]{images/brain_sweep.png}
    \caption{Brain-Parametersuche: Cluster-Anzahl als Funktion von \texttt{BRAIN\_THRESHOLD} auf einer Stichprobe von $4\,000$ Lognachrichten mit der in Listing~\ref{lst:brain-regex} gezeigten Regex- und Delimiter-Konfiguration. Die blaue Strichpunktlinie markiert die $42$ Ground-Truth-Klassen der Stichprobe; die rote gestrichelte Linie zeigt den nächstgelegenen getesteten Wert (\texttt{threshold} $=1$, $72$ Cluster).}
    \label{fig:brain-sweep}
\end{figure}

\paragraph{Beobachtung 1: monoton wachsende Treppenfunktion mit Plateaus an beiden Rändern.}
Die Cluster-Anzahl steigt in Abbildung~\ref{fig:brain-sweep} monoton von $72$
(\texttt{threshold} $\in \{1, 2\}$) auf $159$ (\texttt{threshold} $\in \{15,
20\}$). An beiden Rändern des Suchraums bildet sich ein Plateau: Zwischen
\texttt{threshold} $=1$ und $=2$ sowie zwischen \texttt{threshold} $=15$ und
$=20$ ändert sich die Cluster-Anzahl nicht. Im mittleren Bereich
...
95$, $+31{,}9\,\%$), einem zweiten deutlichen Sprung zwischen
\texttt{threshold} $=7$ und $=8$ ($118 \to 138$, $+16{,}9\,\%$) und einem
weiteren Plateau zwischen \texttt{threshold} $=6$ und $=7$ ($118$ Cluster).
...
insgesamt das $2{,}2$-fache des Startwerts.

\paragraph{Beobachtung 2: Das GT-Distanz-Kriterium liefert kein inneres Optimum.}
Bereits der kleinste getestete Wert \texttt{threshold} $=1$ erzeugt mit $72$
Clustern eine Abweichung von $\Delta = 30$ zur Ground-Truth-Klassenzahl der
Stichprobe ($42$) -- das Fünfzehnfache der bei Drain3 erreichten
...
\begin{figure}[h]
    ...
    \texttt{THRESHOLD} ($n = 2\,000$). Globales Maximum des
...
Mit steigendem \texttt{THRESHOLD} sinkt die Cluster-Anzahl monoton: $101$
($\texttt{THRESHOLD} = 0{,}1$), $53$ ($0{,}2$), $37$ ($0{,}3$), $20$
($0{,}4$), $15$ ($0{,}5$), $8$ ($0{,}6$), $7$ ($0{,}7$). Der
Silhouette-Score erreicht bei \texttt{THRESHOLD} $= 0{,}3$ sein globales
Maximum von $0{,}730$ und liegt für $0{,}1$ ($0{,}710$) und $0{,}2$
($0{,}722$) nur knapp darunter -- die feinere Aufteilung in mehr Cluster
bleibt also intern kohärent. Ab \texttt{THRESHOLD} $= 0{,}4$ ($0{,}672$)
fällt der Score deutlicher und erreicht bei $0{,}5$ mit $0{,}603$ sein
Minimum, bevor er bei $0{,}6$ ($0{,}615$) und $0{,}7$ ($0{,}611$) auf
niedrigem Niveau leicht schwankt.

\paragraph{Wahl des Parameters.}
Datengetrieben ergibt sich \texttt{THRESHOLD} $= 0{,}3$ aus dem Maximum des
Silhouette-Scores. Die resultierenden $37$ Cluster auf der Stichprobe liegen
