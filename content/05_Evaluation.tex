...
der Distortion sowie den Silhouette-Score. Beide Metriken operieren
ausschließlich auf den Eingangsvektoren und sind damit auch im
...
werden Distortion und Silhouette-Score berechnet. Die zugehörige
...
\begin{figure}[h]
    ...
    \caption{Elbow-Diagramm der Distortion über $k \in [2, 39]$. Der Knick liegt im Bereich $k \in \{10, 11, 12\}$.}
...
\begin{figure}[h]
    ...
    \caption{Silhouette-Score über $k \in [2, 39]$. Globales Maximum bei $k = 10$ ($\approx 0{,}216$).}
    \label{fig:kmeans-silhouette}
\end{figure}

\paragraph{Auswertung.}
Abbildung~\ref{fig:kmeans-elbow} zeigt einen deutlichen Knick im Bereich
$k = 10$ bis $k = 12$: Die Distortion fällt zwischen $k = 2$ und $k = 10$
steil von $\approx 0{,}67$ auf $\approx 0{,}49$, anschließend nur noch
marginal. In Abbildung~\ref{fig:kmeans-silhouette} liegt das globale Maximum
des Silhouette-Scores bei $k = 10$ mit $\approx 0{,}216$. Die späteren
lokalen Maxima bei $k \approx 37$ entsprechen typischen Artefakten der
Über-Partitionierung -- das Distortion-Diagramm verläuft in diesem Bereich
bereits flach -- und stellen kein echtes Optimum dar.

Beide Heuristiken konvergieren auf $k = 10$. Dieser Wert wird daher für die
...
Silhouette-Scores ist mit Werten unterhalb von $0{,}25$ gering, was bereits
an dieser Stelle auf eine schwache geometrische Trennung der Cluster im
TF-IDF-Raum hinweist; diese Beobachtung wird in der Diskussion der
K-Means-Ergebnisse (Abschnitt~\ref{sec:eval:kmeans}) erneut aufgegriffen.
...
\begin{table}[h]
    ...
    \begin{tabular}{lrr}
        ...
        \midrule
        ARI            & $56{,}5\,\%$ & $\pm\,8{,}2\,\%$ \\
        V-Measure      & $78{,}2\,\%$ & $\pm\,3{,}8\,\%$ \\
        Homogenität    & $83{,}1\,\%$ & $\pm\,3{,}8\,\%$ \\
        Vollständigkeit& $74{,}0\,\%$ & $\pm\,3{,}9\,\%$ \\
        \midrule
        Clusteranzahl  & $10$ & -- \\
        \bottomrule
    \end{tabular}
...
Die Clusteranzahl $k = 10$ wurde, wie in Abschnitt~\ref{sec:real:kmeans}
beschrieben, datengetrieben über die Elbow-Methode und den Silhouette-Score
bestimmt; sie entspricht damit dem unsupervised Optimum auf der
SVD-reduzierten TF-IDF-Repräsentation. Trotz der gegenüber der Ground-Truth
($44$ Klassen) deutlich geringeren Cluster\-anzahl erreicht das Verfahren mit
einem ARI von $56{,}5\,\%$ und einem V-Measure von $78{,}2\,\%$ einen der
höheren Werte unter den algorithmischen Verfahren – knapp unterhalb von
Drain3 (vgl.\ Abschnitt~\ref{sec:eval:vergleich}).

Bemerkenswert ist die im Vergleich zu Homogenität und Vollständigkeit
ausgewogenere Verteilung beider Teilmetriken ($83{,}1\,\%$ vs.\ $74{,}0\,\%$).
Die Cluster sind weder stark übersegmentiert noch in extremem Maße
zusammengefasst; die Differenz von rund neun Prozentpunkten zugunsten der
Homogenität deutet aber darauf hin, dass die $10$ Partitionen
\emph{Makrostrukturen} des gemischten Datensatzes erfassen – etwa die
...
Die Standardabweichung von $\pm\,8{,}2$ Prozentpunkten beim ARI ist die
